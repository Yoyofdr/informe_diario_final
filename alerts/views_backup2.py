...
def registro_prueba(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Si todo está OK, proceder con el registro con reintentos
            import time
            max_intentos = 3
            intento_actual = 0
            ...
            while intento_actual < max_intentos and not registro_exitoso:
                intento_actual += 1
                print(f"[REGISTRO] Intento {intento_actual} de {max_intentos} para {email}")
                ...
                # VERIFICACIÓN FINAL: Confirmar que el usuario puede hacer login
                try:
                    ...
                    if not user_final or not org_final:
                        raise Exception(f"Error de verificación: Usuario o organización no encontrados después de registro")
                    
                    print(f"[REGISTRO] Verificación final exitosa - Usuario ID: {user_final.id}, Org ID: {org_final.id}")
                    
                except Exception as e:
                    print(f"[REGISTRO] ERROR en verificación final: {e}")
                    # Si llegamos aquí, algo salió muy mal
                    messages.error(request, "Hubo un problema al completar tu registro. Por favor, intenta nuevamente.")
                    return render(request, 'alerts/registro_prueba_chennai.html', {'form': form})
                
                # Solo mostrar éxito si TODO está confirmado
